...
check_dart() {
    ...
    if [[ ! -f "$mcp_wrapper" ]]; then
        log_error "MCP Dart wrapper not found: $mcp_wrapper"
        return 1
    fi
    
    if [[ ! -x "$mcp_wrapper" ]]; then
        log_error "MCP Dart wrapper is not executable: $mcp_wrapper"
        return 1
    fi
...
# Execute the Dart script via MCP server
run_dart_generator() {
    log_verbose "Executing Dart data generation script via MCP server..."
    ...
    if [[ ${#dart_args[@]} -gt 0 ]]; then
        log_verbose "Running: $mcp_wrapper $DART_SCRIPT ${dart_args[*]}"
    else
        log_verbose "Running: $mcp_wrapper $DART_SCRIPT"
    fi
    
    # Execute the Dart script via MCP wrapper
    if "$mcp_wrapper" "$DART_SCRIPT" "${dart_args[@]}"; then
        return 0
    else
        local exit_code=$?
