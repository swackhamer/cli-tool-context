...
## Overview
This comprehensive reference documents 310+ essential CLI tools for Claude's programming and system administration tasks. The tools are organized by functional categories with descriptions and practical examples covering everything from basic file operations to advanced development workflows.

**Total Tools Documented**: 310+ essential commands
**Coverage**: Complete reference for programming, system administration, networking, security, development, cloud infrastructure, media processing, data analysis, database management, and modern alternatives
**Last Updated**: Phase 8D additions - modern alternatives (procs, dust, sd), security tools (masscan), build systems (ninja, meson, bazel), specialized categories and enhanced navigation
...
**Description**: Create, modify, and extract from archives (libraries)
**Location**: `/usr/bin/ar`
**Common Use Cases**:
- Create static libraries
- Manage object file archives
- Library development
- Legacy archive handling

**See Also**: `ranlib` (archive indexing), `nm` (symbol listing), `objdump` (object analysis)
...
# Verbose operations
ar tv libmylib.a  # detailed listing
ar rv libmylib.a file.o  # verbose add

# Replace specific file
ar r libmylib.a file1.o

# Quick append (for efficiency)
ar q libmylib.a *.o
```

### **ranlib** - Archive Index Generator
**Description**: Generate index for archive to speed up linking
**Location**: `/usr/bin/ranlib`
**Common Use Cases**:
- Index static libraries
- Optimize library access
- Library development workflow
- Build system integration

**See Also**: `ar` (archive creation), `nm` (symbol listing), `ld` (linker)

**Examples**:
```bash
# Generate index for library
ranlib libmylib.a

# Check if archive needs indexing
ranlib -t libmylib.a

# Process multiple libraries
ranlib *.a

# Use with ar for complete library creation
ar rcs libmylib.a *.o
ranlib libmylib.a

# Verify library is properly indexed
nm -s libmylib.a | head
```

### **size** - Display Object File Sizes
**Description**: Display sizes of sections in object files
**Location**: `/usr/bin/size`
**Common Use Cases**:
...
**See Also**: `nm` (symbol listing), `objdump` (object analysis), `strip` (remove symbols)

**Examples**:
```bash
# Show sizes of object file
size program

# Show sizes with headers
size -m program
...
# Different radix
size -o program  # octal
size -x program  # hexadecimal
...
**Description**: Force completion of pending disk writes
**Location**: `/bin/sync`
**Common Use Cases**:
- Ensure data is written to disk
- Safe system shutdown preparation
- Data integrity operations
- USB/external drive safety

**See Also**: `fsync` (specific file sync), `umount` (unmount with sync), `diskutil` (macOS disk operations)
...
# Use before unmounting drives
sync && umount /dev/disk1

# Ensure data safety before shutdown
sync; sync; sync

# In scripts for data integrity
cp important_data.txt backup/
sync
echo "Data safely written to disk"

# Before ejecting USB drives
sync && diskutil eject disk2
```
...
**See Also**: `wait` (wait for processes), `timeout` (command timeout), `at` (scheduled execution)
...
sleep 3.5  # 3.5 seconds

# Use in loops for rate limiting
for file in *.txt; do
    process_file "$file"
    sleep 1  # 1 second between files
done

# Retry logic with backoff
attempt=1
while ! command_that_might_fail; do
    echo "Attempt $attempt failed, retrying in $attempt seconds..."
    sleep $attempt
    ((attempt++))
...
**See Also**: `xargs` (modern alternative), `parallel` (parallel execution), `find -exec` (find with execute)

**Examples**:
```bash
# Apply command to multiple arguments
apply echo file1 file2 file3

# Use with file lists
apply 'cp %1 backup/' *.txt

# Transform filenames
apply 'mv %1 %1.bak' *.log

# Process pairs of arguments
apply 'diff %1 %2' old_file1 new_file1 old_file2 new_file2

# Create multiple directories
apply 'mkdir -p project/%1' feature1 feature2 feature3
...
# Modern alternative using xargs (preferred)
echo -e "file1\nfile2\nfile3" | xargs -I {} cp {} backup/
```

### **true** - Return True
**Description**: Do nothing successfully (always returns exit code 0)
**Location**: Built-in command
**Common Use Cases**:
- Infinite loops
- Default case in conditionals
...
**See Also**: `false` (return false), `:` (null command), `exit` (exit with code)
...
# Reset exit status
false; true; echo $?  # outputs 0
```

### **false** - Return False
**Description**: Do nothing unsuccessfully (always returns exit code 1)
**Location**: Built-in command
**Common Use Cases**:
- Testing error conditions
- Forcing script failures
- Loop termination
- Conditional testing

**See Also**: `true` (return true), `exit` (exit with code), `test` (conditional testing)

**Examples**:
```bash
# Test error handling
false && echo "This won't run" || echo "This will run"

# Force script to exit with error
if [ "$CRITICAL_ERROR" = "yes" ]; then
...
# Set exit status for testing
false; echo $?  # outputs 1

# Disable a section temporarily
if false; then
    # This entire block is disabled
    dangerous_operation
    cleanup_after_danger
fi
```
